...
class UserViewController: UIViewController, UIScrollViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        setupScrollView()
        setupHeaderView(color: .searchBarTintColor,
                        height: headerHeight,
                        presenter: presenter,
    ...
    func setupGeneralView(_ user: User) {
    ...
    func setupEducationView(_ education: [Education]) {
    ...
    func setupCareerView(_ job: [Job]) {
    ...
    func setupInterestsView(_ interests: [ProfileInterest]) {
        interestsView = ProfileInterestsView(interests: interests)
        interestsView.delegate = self
        let swipeLeft = UISwipeGestureRecognizer()
    ...
    @objc private func editButtonPressed() {
        let popoverContentController = UserPopoverController()
...
// MARK: - UserPopoverDelegate
extension UserViewController: UserPopoverDelegate {
    
    func addContact() {
        guard let userId = userId else { return }
        presenter?.addContact(with: userId)
    }
...
//        presenter?.getStatusUser()
    ...
    func blockContact() {
        guard let userId = userId else { return }
        presenter?.addToBlockList(with: userId)
    }
}
